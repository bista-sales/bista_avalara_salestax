...
#            and c_code in cs_code:
                ...
                if avatax_config.on_line:
                    ...
                    for line1 in lines1:
                        ...
                        order_line.write(cr, uid, [line1['id']], {'tax_amt': ol_tax_amt, 'tax_id': [(6,0, tax_id)]})
                
                    #tax based on individual shipping order line
                    for line2, s_line in zip(lines2, order.shipping_lines):
                        sl_tax_amt = account_tax_obj._get_compute_tax(cr, uid, avatax_config, order_date,
                                                                    order.name, 'SalesOrder', order.partner_id, ship_from_address_id,
                    ...
                    tax_amount = account_tax_obj._get_compute_tax(cr, uid, avatax_config, order_date,
                                                                    order.name, 'SalesOrder', order.partner_id, ship_from_address_id,
